                ...
                if (empty($upLoadedRequest->collectionType) or $upLoadedRequest->collectionType == "") {
                    ...
                    if ($fitbitApp->isUser($upLoadedRequest->ownerId)) {
                        ...
                        if (strtotime($cooldown) < strtotime(date("Y-m-d H:i:s"))) {
                            ...
                            if ($fitbitApp->getDatabase()->has($fitbitApp->getSetting("db_prefix", NULL, FALSE) . "runlog", array("AND" => array("user" => $upLoadedRequest->ownerId, "activity" => $upLoadedRequest->collectionType)))) {
                                $fields = array(
                            ...
                            if ($upLoadedRequest->collectionType == "foods") {
                                if ($fitbitApp->getDatabase()->has($fitbitApp->getSetting("db_prefix", NULL, FALSE) . "runlog", array("AND" => array("user"     => $upLoadedRequest->ownerId,
                                                                                                                                                     "activity" => "water")))
                                ) {
                                    $fields = array(
                                        "date"     => date("Y-m-d H:i:s"),

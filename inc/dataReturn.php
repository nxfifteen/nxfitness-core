<?php
	...
	class dataReturn {
		...
		public function returnUserRecordBadges() {
			$userBadges = $this->getAppClass()->getDatabase()->select( $this->getAppClass()->getSetting( "db_prefix", NULL, FALSE ) . "bages_user", array(
				...
				array(
					...
					$this->getAppClass()->getSetting( "db_prefix", NULL, FALSE ) . "bages_user.fuid" => $this->getUserID(),
					"ORDER"                                                                          => array(
		...
		public function returnUserRecordDashboard() {
			// Achivment
			$dbSteps     = $this->getAppClass()->getDatabase()->get( $this->getAppClass()->getSetting( "db_prefix", NULL, FALSE ) . "steps", array(
				'distance',
				'floors',
				'steps',
				'syncd'
			), $this->dbWhere() );
			$dbStepsGoal = $this->getAppClass()->getDatabase()->get( $this->getAppClass()->getSetting( "db_prefix", NULL, FALSE ) . "steps_goals", array(
				'distance',
				'floors',
				'steps'
			), $this->dbWhere() );
		...
		public function returnUserRecordDevices() {
			...
			foreach ( $dbDevices as $key => $dev ) {
				...
				$dbDevices[ $key ]['charges'] = $this->getAppClass()->getDatabase()->count( $this->getAppClass()->getSetting( "db_prefix", NULL, FALSE ) . "devices_charges", array(
					"AND" => array(
						"charged" => 1,
						"id"      => $dbDevices[ $key ]['id']
					)
		...
		public function returnUserRecordFoodDiary() {
			...
			if ( ! array_key_exists( "LIMIT", $where ) OR $where['LIMIT'] == 1 ) {
				...
				$returnArray['water'] = array(
					"liquid" => (String) round( $dbWater, 2 ),
					"goal"   => ( $this->getAppClass()->getUserSetting( $this->getUserID(), "goal_water", '200' ) * $where['LIMIT'] )
				);
			}
					...
					foreach ( $dbLegs as $dbLeg ) {
						...
						$dbNarratives = $this->getAppClass()->getDatabase()->select( $this->getAppClass()->getSetting( "db_prefix", NULL, FALSE ) . "journeys_narrative", array(
							...
							array(
								...
								"LIMIT" => 1,
								"ORDER" => $this->getAppClass()->getSetting( "db_prefix", NULL, FALSE ) . "journeys_narrative.miles DESC"
							) );
						$this->getAppClass()->getErrorRecording()->postDatabaseQuery( $this->getAppClass()->getDatabase(), array(
							"METHOD" => __METHOD__,
							"LINE"   => __LINE__
						...
						$dbNarrativeNext = $this->getAppClass()->getDatabase()->select( $this->getAppClass()->getSetting( "db_prefix", NULL, FALSE ) . "journeys_narrative", array(
							...
							array(
								...
								"LIMIT" => 1,
								"ORDER" => $this->getAppClass()->getSetting( "db_prefix", NULL, FALSE ) . "journeys_narrative.miles ASC"
							) );
						$this->getAppClass()->getErrorRecording()->postDatabaseQuery( $this->getAppClass()->getDatabase(), array(
							"METHOD" => __METHOD__,
							"LINE"   => __LINE__
		...
		public function returnUserRecordKeyPoints() {
			...
			$dbUsers = $this->getAppClass()->getDatabase()->get( $this->getAppClass()->getSetting( "db_prefix", NULL, FALSE ) . "users", array(
				'rank',
				'friends',
				'distance',
				'gender'
			 ...
			 * Set Max values
			 */
			$dbMaxSteps = $this->getAppClass()->getDatabase()->get( $this->getAppClass()->getSetting( "db_prefix", NULL, FALSE ) . "steps", array(
				'steps',
				'date'
			...
			$dbMaxDistance = $this->getAppClass()->getDatabase()->get( $this->getAppClass()->getSetting( "db_prefix", NULL, FALSE ) . "steps", array(
				'distance',
				'date'
			...
			$dbMaxFloors = $this->getAppClass()->getDatabase()->get( $this->getAppClass()->getSetting( "db_prefix", NULL, FALSE ) . "steps", array(
				'floors',
				'date'
			...
			$dbMaxElevation = $this->getAppClass()->getDatabase()->get( $this->getAppClass()->getSetting( "db_prefix", NULL, FALSE ) . "steps", array(
				'elevation',
				'date'
		...
		public function returnUserRecordSleep() {
			...
			$dbSleepRecords = $this->getAppClass()->getDatabase()->select( $this->getAppClass()->getSetting( "db_prefix", NULL, FALSE ) . "sleep", array(
				...
				array(
					...
					$this->getAppClass()->getSetting( "db_prefix", NULL, FALSE ) . "sleep_user.user" => $this->getUserID(),
		...
		public function returnUserRecordTasker() {
			...
			$dbGoals = $this->getAppClass()->getDatabase()->select( $this->getAppClass()->getSetting( "db_prefix", NULL, FALSE ) . "steps_goals", array(
				'distance',
				'floors',
				'steps',
				'syncd'
		...
		public function returnUserRecordGoalStreak() {
			...
			if ( $this->getAppClass()->getDatabase()->has( $this->getAppClass()->getSetting( "db_prefix", NULL, FALSE ) . "streak_goal",
				...
				$currentStreakStart = $this->getAppClass()->getDatabase()->get( $this->getAppClass()->getSetting( "db_prefix", NULL, FALSE ) . "streak_goal", array(
					"start_date",
					"length"
				),
					array( "AND" => array( "fuid" => $this->getUserID(), "goal" => "steps", "end_date" => NULL ) ) );
			...
			if ( $this->getAppClass()->getDatabase()->has( $this->getAppClass()->getSetting( "db_prefix", NULL, FALSE ) . "streak_goal",
				...
				$databaseResults = $this->getAppClass()->getDatabase()->get( $this->getAppClass()->getSetting( "db_prefix", NULL, FALSE ) . "streak_goal", array(
					"start_date",
					"end_date",
					"length"
				),
		...
		public function returnUserRecordTrend() {
			...
			$dbBody                           = $this->getAppClass()->getDatabase()->get( $this->getAppClass()->getSetting( "db_prefix", NULL, FALSE ) . "body", array(
				'date',
				'weight',
				'weightGoal',
				'fat',
			...
			$dbGoalsCalories              = $this->getAppClass()->getDatabase()->get( $this->getAppClass()->getSetting( "db_prefix", NULL, FALSE ) . "food_goals", array( 'estimatedDate' ), array(
				"user"  => $this->getUserID(),
				"ORDER" => "date DESC"
			) );
			...
			$dbSteps = $this->getAppClass()->getDatabase()->get( $this->getAppClass()->getSetting( "db_prefix", NULL, FALSE ) . "steps", array( 'caloriesOut' ), array(
				"user"  => $this->getUserID(),
				"ORDER" => "date DESC"
			) );
			$dbfood  = $this->getAppClass()->getDatabase()->sum( $this->getAppClass()->getSetting( "db_prefix", NULL, FALSE ) . "food", array( 'calories' ), array(
				"AND"   => array(
					"user" => $this->getUserID(),
					"date" => $this->getParamDate()
				),
		...
		public function returnUserRecordWeight() {
			...
			if ( count( $dbWeight ) == 0 ) {
				...
				foreach ( $period as $dt ) {
					/** @var DateTime $dt */
					$returnWeight[ $dt->format( "Y-m-d" ) ] = array(

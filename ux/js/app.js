...
function loadJS(jsFiles, pageScript) {
    ...
    for (i = 0; i < jsFiles.length; i++) {
        ...
        jsFiles[i] = jsFiles[i].replace('bower_components', bowerPath);
    ...
    if (pageScript) {
        ...
        //noinspection JSUnresolvedVariable
        pageScript = pageScript.replace('bower_components', bowerPath);
...
function loadCSS(cssFile, end, callback) {
    ...
    if (!cssArray[cssFile]) {
        ...
        if (end === 1) {
            ...
            //noinspection JSUnresolvedVariable
            cssFile = cssFile.replace('bower_components', bowerPath);
